...
// Server:
    ...
    // ADVANCED SETTINGS:
		...
		// What uniforms are forbidden for marines (people spawning in water). Forbidden because CSWR can't handle them properly when the unit behaves as a diver. These uniforms force unit to be a diver:
		// Important: this is case-sensitive, so be carefull with any typo!
		CSWR_forbiddenMarineUniforms = ["U_B_Wetsuit","U_O_Wetsuit","U_I_Wetsuit","U_B_survival_uniform"];
		// Faces by region:
		// Important: this is case-sensitive, so be carefull with any typo!
