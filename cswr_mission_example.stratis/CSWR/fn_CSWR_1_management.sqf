...
// File: your_mission\CSWR\fn_CSWR_1_management.sqf
...
// Debug:
    CSWR_isOnDebug = true;               // true = shows basic debug information for the Mission Editor / false = turn it off / Default: false;
        ...
        CSWR_isOnDebugSectors = false;   // true = shows deeper info about sectors / false = turn it off / Default: false;
        CSWR_isOnDebugSupport = false;   // true = shows deeper info about support resources if available / false = turn it off / Default: false;
...
// Sides:
    ...
    CSWR_isOnIND = true;  // true = spawn Indepdentent/Resistence through CSWR / false = don't spawn this side.
    CSWR_isOnCIV = false;  // true = spawn Civilian through CSWR / false = don't spawn this side.

// Global units settings:
    CSWR_isBackpackForAllByFoot = false;  // true = all units by foot (including CIV if spawned by CSWR) get it / false = only units originally with backpacks / Default: false;
    CSWR_isVestForAll           = false;  // true = all units (including CIV if spawned by CSWR) get it / false = only units originally with vests / Default: false;

// Settings by Side:
    // Blu
        CSWR_facesFromRegionBLU  = "NorthAmerica";       // Options: "Asia", "Africa", "Europe", "EastEurope", "MidEast", "NorthAmerica", "SouthAmerica" / Empty ("") no changes / Default: "";
        CSWR_languageBLU         = "UsEnglish";       // Options: "UsEnglish", "GbEnglish", "French", "Polish", "Russian", "Chinese", "Iranian" / Empty ("") auto selected by region / Default: "";
    ...
    // Opf
        CSWR_facesFromRegionOPF  = "EastEurope";       // Options: "Asia", "Africa", "Europe", "EastEurope", "MidEast", "NorthAmerica", "SouthAmerica" / Empty ("") no changes / Default: "";
        CSWR_languageOPF         = "Russian";       // Options: "UsEnglish", "GbEnglish", "French", "Polish", "Russian", "Chinese", "Iranian" / Empty ("") auto selected by region / Default: "";
    ...
    // Ind
        CSWR_facesFromRegionIND  = "Africa";       // Options: "Asia", "Africa", "Europe", "EastEurope", "MidEast", "NorthAmerica", "SouthAmerica" / Empty ("") no changes / Default: "";
        CSWR_languageIND         = "French";       // Options: "UsEnglish", "GbEnglish", "French", "Polish", "Russian", "Chinese", "Iranian" / Empty ("") auto selected by region / Default: "";
    ...
    // Civ
        CSWR_facesFromRegionCIV = "Asia";           // Options: "Asia", "Africa", "Europe", "EastEurope", "MidEast", "NorthAmerica", "SouthAmerica" / Empty ("") no changes / Default: "";
        CSWR_languageCIV        = "Chinese";           // Options: "UsEnglish", "GbEnglish", "French", "Polish", "Russian", "Chinese", "Iranian" / Empty ("") auto selected by region / Default: "";
...
// Global vehicles settings:
    // true = vehicles spawned by CSWR have their original cargo items removed / false = nothing changed / Default: [true, true, true, true];
    CSWR_removeItemsCargo = [true, true, true, true];  // means [clearWeaponCargo, clearMagazineCargo, clearItemCargo, clearBackpackCargo];
...
// Global Helicopters settings:
    ...
    CSWR_heliLightAlt         = 100;   // In meters, cruising altitude for helicopters of light class / Minimal: 30 / Default: 100;
    CSWR_heliMediumAlt        = 300;   // In meters, cruising altitude for helicopters of medium class / Minimal: 60 more than light class / Default: 300;
    CSWR_heliHeavyAlt         = 600;   // In meters, cruising altitude for helicopters of heavy class / Minimal: 120 more than Medium class / Default: 600;
    CSWR_airTransportWaitTime = 120;   // In seconds, how long the helicopter should wait the player group to get in and get out before take off / Minimal 45 / Default: 120;
...
[] spawn {
	...
	missionNameSpace setVariable ["CSWR_suprtTranspAirData", createHashMapFromArray [[BLUFOR, []],[OPFOR, []],[INDEPENDENT, []],[CIVILIAN, []]], true];
	missionNameSpace setVariable ["CSWR_suprtExtractAirData", createHashMapFromArray [[BLUFOR, []],[OPFOR, []],[INDEPENDENT, []],[CIVILIAN, []]], true];
	CSWR_spwnDelayQueue    = 0;  // debug proposes.
	...
	_expOpts                = ["R","V","E"];
	// Global escape:
	if !CSWR_isOn exitWith {publicVariable "CSWR_isOn"; publicVariable "CSWR_isOnDebug"; if CSWR_isOnDebug then {systemChat format ["%1 The %2 script was turned off manually through 'fn_CSWR_1_management.sqf' file.", CSWR_txtDebugHeader, CSWR_prefix]}};
	// Debug txts:
	_txt1="For good combat experince, don't use"; _txt2="value less than"; _txt3="out of debug mode. Minimal value"; _txt4="GEAR > NIGHTVISION > You turned the NVG usage 'true' for"; _txt5="side, but in parallel you're trying to force removal of"; _txt6="NVG's. Fix it in 'fn_CSWR_1_management.sqf' file. Generic NVG was applied."; _txt7="GEAR > FLASHLIGHT > You turned the Flashlight usage 'true' for"; _txt8="FLASHLIGHTS. Fix it in 'fn_CSWR_1_management.sqf' file. Generic Flashlight was applied."; _txt9="CANNOT be empty in 'fn_CSWR_1_management.sqf' file. Default value has been restored temporarily";
	...
	if ( _confirmedMkrs isEqualTo [] ) exitWith {
	...
	if CSWR_isOnDebug then {
		...
		if ( _debugOccupyInters isNotEqualTo [] ) then {
			systemChat format ["%1 OCCUPY > The follow asset's listed as 'Ignored Buildings' and 'Acceptable Ruins' at the same time in 'fn_CSWR_1_management.sqf'. Fix them: %2", CSWR_txtWarnHeader, str _debugOccupyInters] };
		...
		switch CSWR_facesFromRegionBLU do {
			...
			default {
				if ( CSWR_facesFromRegionBLU isNotEqualTo "" ) then {
					systemChat format ["%1 REGION FACES > The %2 side's using a UNKNOWN region ('%3'). Fix it using one of those options in 'fn_CSWR_1_management.sqf' file.",
	...
	*/
	// BluFor spawns:
	CSWR_spawnsForPeopleBLU     = ((_confirmedMkrs # 0) # 0) # 0;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsLandBLU           = CSWR_spawnsForPeopleBLU # 0;
	CSWR_spwnsWaterBLU          = CSWR_spawnsForPeopleBLU # 1;
	CSWR_spawnsForVehicleBLU    = ((_confirmedMkrs # 0) # 0) # 1;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsVehLandBLU        = CSWR_spawnsForVehicleBLU # 0;
	CSWR_spwnsVehWaterBLU       = CSWR_spawnsForVehicleBLU # 1;
	CSWR_spawnsForHelicopterBLU = ((_confirmedMkrs # 0) # 0) # 2;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsHeliLandBLU       = CSWR_spawnsForHelicopterBLU # 0;
	CSWR_spwnsHeliWaterBLU      = CSWR_spawnsForHelicopterBLU # 1;
	CSWR_spawnsParadropBLU      = ((_confirmedMkrs # 0) # 0) # 3;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsParaAirBLU        = CSWR_spawnsParadropBLU # 0;
	//CSWR_spwnsParaUselessBLU  = CSWR_spawnsParadropBLU # 1;
	CSWR_spwnsAllBLU = CSWR_spwnsLandBLU + CSWR_spwnsWaterBLU + CSWR_spwnsVehLandBLU + CSWR_spwnsVehWaterBLU + CSWR_spwnsHeliLandBLU + CSWR_spwnsHeliWaterBLU + CSWR_spwnsParaAirBLU;  // Used in fn_CSWR_2_population.sqf

	// OpFor spawns:
	CSWR_spawnsForPeopleOPF     = ((_confirmedMkrs # 0) # 1) # 0;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsLandOPF           = CSWR_spawnsForPeopleOPF # 0;
	CSWR_spwnsWaterOPF          = CSWR_spawnsForPeopleOPF # 1;
	CSWR_spawnsForVehicleOPF    = ((_confirmedMkrs # 0) # 1) # 1;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsVehLandOPF        = CSWR_spawnsForVehicleOPF # 0;
	CSWR_spwnsVehWaterOPF       = CSWR_spawnsForVehicleOPF # 1;
	CSWR_spawnsForHelicopterOPF = ((_confirmedMkrs # 0) # 1) # 2;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsHeliLandOPF       = CSWR_spawnsForHelicopterOPF # 0;
	CSWR_spwnsHeliWaterOPF      = CSWR_spawnsForHelicopterOPF # 1;
	CSWR_spawnsParadropOPF      = ((_confirmedMkrs # 0) # 1) # 3;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsParaAirOPF        = CSWR_spawnsParadropOPF # 0;
	//CSWR_spwnsParaUselessOPF  = CSWR_spawnsParadropOPF # 1;
	CSWR_spwnsAllOPF = CSWR_spwnsLandOPF + CSWR_spwnsWaterOPF + CSWR_spwnsVehLandOPF + CSWR_spwnsVehWaterOPF + CSWR_spwnsHeliLandOPF + CSWR_spwnsHeliWaterOPF + CSWR_spwnsParaAirOPF;  // Used in fn_CSWR_2_population.sqf
	
	// Independent spawns:
	CSWR_spawnsForPeopleIND     = ((_confirmedMkrs # 0) # 2) # 0;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsLandIND           = CSWR_spawnsForPeopleIND # 0;
	CSWR_spwnsWaterIND          = CSWR_spawnsForPeopleIND # 1;
	CSWR_spawnsForVehicleIND    = ((_confirmedMkrs # 0) # 2) # 1;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsVehLandIND        = CSWR_spawnsForVehicleIND # 0;
	CSWR_spwnsVehWaterIND       = CSWR_spawnsForVehicleIND # 1;
	CSWR_spawnsForHelicopterIND = ((_confirmedMkrs # 0) # 2) # 2;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsHeliLandIND       = CSWR_spawnsForHelicopterIND # 0;
	CSWR_spwnsHeliWaterIND      = CSWR_spawnsForHelicopterIND # 1;
	CSWR_spawnsParadropIND      = ((_confirmedMkrs # 0) # 2) # 3;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsParaAirIND        = CSWR_spawnsParadropIND # 0;
	//CSWR_spwnsParaUselessIND  = CSWR_spawnsParadropIND # 1;
	CSWR_spwnsAllIND = CSWR_spwnsLandIND + CSWR_spwnsWaterIND + CSWR_spwnsVehLandIND + CSWR_spwnsVehWaterIND + CSWR_spwnsHeliLandIND + CSWR_spwnsHeliWaterIND + CSWR_spwnsParaAirIND;  // Used in fn_CSWR_2_population.sqf
	
	// Civilian spawns:
	CSWR_spawnsForPeopleCIV     = ((_confirmedMkrs # 0) # 3) # 0;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsLandCIV           = CSWR_spawnsForPeopleCIV # 0;
	CSWR_spwnsWaterCIV          = CSWR_spawnsForPeopleCIV # 1;
	CSWR_spawnsForVehicleCIV    = ((_confirmedMkrs # 0) # 3) # 1;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsVehLandCIV        = CSWR_spawnsForVehicleCIV # 0;
	CSWR_spwnsVehWaterCIV       = CSWR_spawnsForVehicleCIV # 1;
	CSWR_spawnsForHelicopterCIV = ((_confirmedMkrs # 0) # 3) # 2;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsHeliLandCIV       = CSWR_spawnsForHelicopterCIV # 0;
	CSWR_spwnsHeliWaterCIV      = CSWR_spawnsForHelicopterCIV # 1;
	CSWR_spawnsParadropCIV      = ((_confirmedMkrs # 0) # 3) # 3;  // Used in fn_CSWR_2_population.sqf
	CSWR_spwnsParaAirCIV        = CSWR_spawnsParadropCIV # 0;
	//CSWR_spwnsParaUselessCIV  = CSWR_spawnsParadropCIV # 1;
	CSWR_spwnsAllCIV = CSWR_spwnsLandCIV + CSWR_spwnsWaterCIV + CSWR_spwnsVehLandCIV + CSWR_spwnsVehWaterCIV + CSWR_spwnsHeliLandCIV + CSWR_spwnsHeliWaterCIV + CSWR_spwnsParaAirCIV;  // Used in fn_CSWR_2_population.sqf
	// All spawns:
	...
	CSWR_grpTypesForSpwns            = ["teamL","teamM","teamH","teamC1","teamC2","teamC3","teamS"];
	CSWR_grpTypesForSpwnsVehGround   = ["vehL","vehM","vehH","vehC1","vehC2","vehC3"];
	CSWR_grpTypesForSpwnsVehNautical = ["nauL","nauM","nauH"];
	CSWR_grpTypesForSpwnsVehAirHeli  = ["heliL","heliM","heliH"];
	CSWR_grpTypesForSpwnsPara        = CSWR_grpTypesForSpwns + CSWR_grpTypesForSpwnsVehGround;
	...
	*/
	...
	_allDestsBLU         = _allDestRestrictBLU + _allDestWatchBLU + _allDestOccupyBLU + _allDestHoldBLU /* + _allDestTranspBLU */;  // NEVER include PUBLICs in this calc!
	...
	_allDestsOPF         = _allDestRestrictOPF + _allDestWatchOPF + _allDestOccupyOPF + _allDestHoldOPF /* + _allDestTranspOPF */;  // NEVER include PUBLICs in this calc!
	...
	_allDestsIND         = _allDestRestrictIND + _allDestWatchIND + _allDestOccupyIND + _allDestHoldIND /* + _allDestTranspIND */;  // NEVER include PUBLICs in this calc!
	...
	_allDestsCIV         = _allDestRestrictCIV + _allDestWatchCIV + _allDestOccupyCIV + _allDestHoldCIV /* + _allDestTranspCIV */;  // NEVER include PUBLICs in this calc!
	...
	_allDestsSpecial = _allDestWatchBLU  + _allDestWatchOPF  + _allDestWatchIND  + _allDestWatchCIV  +    // watch
	                   ...
	                   // DestTransport doesn't need markers;
	                   // DestExtraction desn't need markers;
